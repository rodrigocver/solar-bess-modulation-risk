...
5. **Given** the CSV solar profile contains a row with a non-numeric or negative value,
   **When** loading is attempted, **Then** the tool identifies the exact row number and
   value, rejects the file, and exits with a descriptive error.
...
5. **Given** a charging hour, **When** results are examined, **Then**
   `charge_h = min(excess_h, bess_power_mw, remaining_capacity_mwh)` and
...
`annual_exposure_without_bess = garantia_fisica_mw × 4 × 500` (4 peak hours × 365 days)
...
2. **Given** hourly PLD prices from BigQuery, **When** exposure is computed, **Then**
   `annual_exposure_without_bess = Σ(garantia_fisica_mw × PLD_h)` summed over all peak
   hours h (those with hour-of-day in the scenario's peak_hours set) across 8,760 h.
...
- **FR-001**: The tool MUST present each configurable parameter with its default value
  ...
  (200), USD/BRL exchange rate (5.0), useful life years (20), discount rate %/yr (10).
...
- **FR-002**: The tool MUST require a solar generation CSV file (8,760 rows, one numeric
  non-negative value per row, power in MW). No synthetic fallback. If the file is absent
  or invalid, the run MUST abort with a descriptive error message.
...
- **FR-005**: The tool MUST simulate exactly three fixed scenarios:
  - Scenario A: peak_hours = {18, 19},         duration = 2 h
  - Scenario B: peak_hours = {17, 18, 19},     duration = 3 h
  - Scenario C: peak_hours = {17, 18, 19, 20}, duration = 4 h
  For each scenario:
...
- **FR-006**: BESS dispatch MUST enforce hour-by-hour the following rules:
  ...
  **Charging (hours where generation_h > garantia_fisica_mw)**:
  - `excess_h = max(0, generation_h − garantia_fisica_mw)`
  - `charge_h = min(excess_h, bess_power_mw, remaining_capacity_mwh)`
  ...
  **Hard bounds**: SoC ∈ [0, bess_energy_mwh]; power flow ≤ bess_power_mw;
...
- **FR-007**: For each scenario the tool MUST compute and report all 10 output metrics:
  ...
  - `bess_power_mw` (MW)
  - `capex_brl` (BRL)
  - `annual_exposure_without_bess` = `Σ(garantia_fisica_mw × PLD_h)` for all h where
    hour-of-day is in peak_hours (BRL/yr)
  - `annual_exposure_with_bess` = `Σ(residual_deficit_h × PLD_h)` for the same hours
    (BRL/yr)
  - `annual_savings` = `exposure_without − exposure_with` (BRL/yr)
  - `payback_years` = `capex_brl / annual_savings` (years, or "não atingível" if
    annual_savings ≤ 0)
...
- **FR-010**: The HTML report MUST be self-contained (all JavaScript, CSS, and chart data
  ...
  Modelo" section listing all configured parameters and documented assumptions. All charts
  MUST use Plotly.js with `include_plotlyjs='inline'` — no CDN references.
...
- **HTML Report**: Self-contained file with four interactive Plotly charts, summary
...
- **SC-004**: `annual_exposure_without_bess` for any scenario, computed via the formula
  in FR-007, matches the sum of `(garantia_fisica_mw × PLD_h)` over peak hours to
...
- BESS capital cost is modelled as a single upfront payment in USD/kWh converted to BRL.
  O&M costs are not modelled. Cumulative savings are undiscounted (simple payback model).
